...
iface_entry interfaces[MAX_INTERFACES];
int iface_count = 0;
...
void find_ifaces(void) {
    ...
    for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
        ...
        if (iface_count < MAX_INTERFACES) {
            strncpy(interfaces[iface_count].name, ifa->ifa_name, IFNAMSIZ);
            interfaces[iface_count].name[IFNAMSIZ - 1] = '\0';
            iface_count++;
        }
    }
    ...
    if (iface_count == 0) {
        fprintf(stderr, "Fant ingen gyldige interfaces!\n");
        exit(1);
    }

    if (debug_mode) {
        printf("[DEBUG] Totalt %d interfaces funnet:\n", iface_count);
